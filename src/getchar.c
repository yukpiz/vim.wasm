...
#ifdef FEAT_GUI_WASM
static void	inchar_async(char_u *buf, int maxlen, long wait_time, void (*callback)(int));
#endif
#ifdef FEAT_EVAL
...
static void (*flush_buffers_callback_)();
static void flush_buffers_after_inchar(int len)
{
    if (len != 0) {
	// Next loop
	inchar_async(typebuf.tb_buf, typebuf.tb_buflen - 1, 10L, flush_buffers_after_inchar);
	return;
    }
    typebuf.tb_off = MAXMAPLEN;
    typebuf.tb_len = 0;
...
void
flush_buffers_async(int flush_typeahead, void (*callback)())
{
    init_typebuf();
...
static void (*vgetorpeek_async_callback_)(int);

static void
vgetorpeek_one_async()
{
/*
 * get a character: 1. from the stuffbuffer
 */
...
# endif
    ...
    vgetorpeek_async_callback_ = callback;
